Project proposals
=================
The project proposals are below are suggestions. You may want to
modify the scope, approach, endpoints, etc.
...
Scientific conference information extraction 
--------------------------------------------
The purpose is to automatically set up scientific conference
information data in Wikidata. The data could be short name, title,
country and town where the conference, conference series, language,
starting time, ending time, key events such as submission deadlines,
organizer by role, speaker, admission (acceptance) rate, accepted
number of papers, number of submissions, number of participants,
program committee members, etc. You can see an example here
https://www.wikidata.org/wiki/Q124784822 for the EKAW 2024 conference.

The project is directly inspired by the article 'Population and
Exploration of Conference Data in Wikidata using LLMs'. 

### Method

Some of the conference information may be present in the 'frontmatter'
of the proceedings. For the EKAW 2024 conference, that would be a PDF
file at
https://link.springer.com/content/pdf/bfm:978-3-031-77792-9/1. For 
instance, a sentence reads "Overall, we received 138 abstract
submissions, of which 115 turned into full paper submissions, which
were reviewed by 123 members of the Programme Committee and 19
additional reviewers." This sentence corresponds to the Wikidata property
P12655 (number of submissions). In this case, we could use,
pdf-to-text -> LLM-based extraction of 'number of submission'.

### Data
There are various sites that record conference data, e.g.,
wikicfp. However, the scope could just be to handle an already defined
conference with a PDF describing the conference. The most consistent
frontmatter PDF files may be those published as 'Lecture Notes in
...
{
    {
        'number_of_submission': 123,
        'quote': 'Overall, we received 138 abstract submissions, of which 115 turned into full paper submissions, which were reviewed by 123 members of the Programme Committee and 19 additional reviewers.',
        'certainty': 0.8, 
    },
    {
        'accepted_papers': 28,
        'quote': 'In total, 28 papers were accepted for publication in this volume',
        'certainty': 0.96,
    },
    {
        'organizer': "Laura Hollink", 
        'role': 'general chair',
        'quote': 'General Chairs Laura Hollink Centrum Wiskunde & Informatica, The Netherlands',
        'wikidata': 'Q102359488',
        'certainty': 0.91
    },
}
...
* Scholarly Wikidata: Population and Exploration of Conference Data in
  Wikidata using LLMs, EKAW 2024,
  https://link.springer.com/chapter/10.1007/978-3-031-77792-9_15 


Extraction of impacts from environmental impact assessment reports
------------------------------------------------------------------
The purpose is to extract 'impacts' from environmental impact
assessment reports. 

'impacts' entails the thing or activity that makes an impact (e.g., a
wind turbine), the affected thing (e.g., neighbors), the impact (e.g.,
noise), possible mitigation, significance, project phase.

Further processing is to entity link extracted phrases to the ontology 
provided, e.g., in our Dreams Wikibase. This can include match to
Sustainable Development Goals.

### Data
- Metadata about the reports is available in a Wikibase:
  https://dreams.wikibase.cloud/ 
- The data in the Wikibase also includes manually extracted impacts,
  see, e.g., https://dreams.wikibase.cloud/wiki/Item:Q1395
- An overview is available at
  https://people.compute.dtu.dk/faan/dreams.html 

The annotation data in the Wikibase can be used to guide, train or
evaluate the system. 

There several issues 
- The data we currently have access to is in Danish.
- The PDF for the reports can be large, several hundreds of pages.
- Many of URLs to the reports are currently wrong.
- Some of references seems to be unavailable from the DTU network!?
- Our manual reference annotation of the 'impact' may be noisy. 
- The ontology is not necessarily complete.

Alternatively there are apparently also the reports available from an
Estonian ministry, see https://kliimaministeerium.ee/piiriulene-moju-hindamine

### References
- Environmental impact assessment reports in Wikidata and a Wikibase
- Do bridges dream of water pollutants? Towards DreamsKG, a knowledge
  graph to make digital access for sustainable environmental
